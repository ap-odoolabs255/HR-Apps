...
.att-ctrl-btn-disabled {
  ...
  cursor: wait !important;
    ...
    // Odoo 19's rpc helper uses fetch, so observe the same endpoint there too.
    (function patchFetch() {
        var originalFetch = window.fetch;
        if (!originalFetch || originalFetch.__attCtrlPatched) return;
        var patchedFetch = function (input, init) {
            var url = typeof input === 'string' ? input : ((input && input.url) || '');
            var isAttendanceRpc = url.indexOf('/hr_attendance/systray_check_in_out') !== -1;

            // The office label already obtained an accurate browser position. Odoo 19
            // otherwise sends false/false when company device tracking is disabled,
            // so the attendance record cannot resolve its office polygon. Reuse the
            // fresh position for this attendance RPC only.
            if (isAttendanceRpc && init && typeof init.body === 'string') {
                try {
                    var cache = window.__att_ctrl_geo_cache;
                    var age = cache ? Date.now() - (cache.ts || 0) : Infinity;
                    if (cache && cache.coords && age >= 0 && age <= 60000) {
                        var payload = JSON.parse(init.body);
                        payload.params = payload.params || {};
                        if (payload.params.latitude === false || payload.params.latitude == null) {
                            payload.params.latitude = cache.coords.latitude;
                        }
                        if (payload.params.longitude === false || payload.params.longitude == null) {
                            payload.params.longitude = cache.coords.longitude;
                        }
                        init = Object.assign({}, init, { body: JSON.stringify(payload) });
                    }
                } catch (e) {
                    // Keep the original RPC body if it is not the expected JSON shape.
                }
            }
            ...
            if (isAttendanceRpc) {
                promise.then(
                    function () { hideLoading('fetch-complete'); },
                    function () { hideLoading('fetch-error'); }
                );
            }
            return promise;
        };
        patchedFetch.__attCtrlPatched = true;
