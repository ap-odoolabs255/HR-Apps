    ...
    def create(self, vals_list):
        ...
        if self._has_postgis() and self._has_geom_column():
            try:
                ...
                _logger.exception("[office.location] Failed to apply geometry during creation")
                raise
        else:
        ...
        if any(k in vals for k in ('geom_wkt', 'geom_srid')):
            if self._has_postgis() and self._has_geom_column():
                try:
                    ...
                    _logger.exception("[office.location] Failed to apply geometry during update")
                    raise
            else:
                _logger.info("[office.location] Skip apply geometry (PostGIS/columns not ready yet).")
        return res
    ...
    def action_apply_geometry(self):
        ...
        for rec in self:
            ...
            try:
                ...
                raise ValidationError(_("Invalid polygon WKT: %s", error)) from error
        # trigger recompute
